...
TASKS_DIR="/Users/tcotd/seb-lab/instagram-advisor/.opencode/tasks"
if [ -d "$TASKS_DIR" ]; then
  STALE_COUNT=$( (find "$TASKS_DIR" -maxdepth 1 -type d -mtime +1 2>/dev/null || true) | wc -l )
  if [ "$STALE_COUNT" -gt 0 ]; then
    echo "⚠️  Found $STALE_COUNT stale task directories in /Users/tcotd/seb-lab/instagram-advisor/.opencode/tasks/"
    echo "   These are leftover from crashed subagent sessions."
    echo "   Run: rm -rf /Users/tcotd/seb-lab/instagram-advisor/.opencode/tasks/*  (safe — only affects dead sessions)"
  fi
fi
```
